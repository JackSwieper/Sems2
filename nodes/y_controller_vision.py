...
class y_controller_node(Node):
    def __init__(self):                             # by initializing the node we are also initializing all relevant variables, publishers and subscriptions
        ...
        self.position_pub       = self.create_publisher (msg_type=Float64Stamped, topic='~/position_y', qos_profile=1)
        # PID-related publishers
        self.setpoint_error_pub = self.create_publisher (msg_type=Float64Stamped, topic='~/setpoint_error_y', qos_profile=1)
        self.p_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/p_gain_y',         qos_profile=1)
        self.i_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/i_gain_y',         qos_profile=1)
        self.d_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/d_gain_y',         qos_profile=1)
                

        # subcription to get the setpoint from setpoint.py publisher
        self.setpoint_sub = self.create_subscription(
            msg_type=Float64Stamped,    
            topic='setpoint_publisher/setpoint/y',                                   
            callback=self.on_setpoint,
            qos_profile=qos)
        # subscription for cheating past the kalman filter
    ...
    def publish_thrust_msg(self, thrust: float, timestamp: rclpy.time.Time) -> None:
        ...
        # we want to set the lateral thrust exlusively. In the bluerovs defined coordinatino system thats x-axis, while in the pool coord. its y. Thats fucked up.
    ...
    def compute_control_output(self) -> float:     # Code of the PID controller
        ...
        if(     (self.position < 0.5 and thrust < 0.0) 
           or   (self.position > 3.5 and thrust > 0.0)):
            return 0.0
