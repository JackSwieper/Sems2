...
from geometry_msgs.msg      import PoseWithCovarianceStamped                            # given class for receiving position



class z_controller_node(Node):
    def __init__(self):                             # by initializing the node we are also initializing all relevant variables, publishers and subscriptions
        super().__init__(node_name ='z_controller')
        ...
        # position-related publisher, only here because this version doesnt apply the kalman filter where the position should be published
        self.position_pub       = self.create_publisher (msg_type=Float64Stamped, topic='~/position_z', qos_profile=1)
        # PID-related publishers
        self.setpoint_error_pub = self.create_publisher (msg_type=Float64Stamped, topic='~/setpoint_error_z', qos_profile=1)
        self.p_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/p_gain_z',         qos_profile=1)
        self.i_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/i_gain_z',         qos_profile=1)
        self.d_gain_pub         = self.create_publisher (msg_type=Float64Stamped, topic='~/d_gain_z',         qos_profile=1)
                

        # subcription to get the setpoint from setpoint.py publisher
        self.setpoint_sub = self.create_subscription(
            msg_type=Float64Stamped,    
            topic='setpoint_publisher/setpoint/z',                                   
            callback=self.on_setpoint,
            qos_profile=qos)
        # subscription to get the current position. TODO switch from vision_pose_cov to kalman filter once its done
    ...
    def on_setpoint(self, setpoint_msg: Float64Stamped) -> None:
        ...
        if  (self.setpoint < -0.8):
           self.setpoint = -0.8
        elif(self.setpoint > -0.1):
            self.setpoint = -0.1            
    
    # callback function when receiving new visual data    
    def on_vision_pose(self, msg: PoseWithCovarianceStamped) -> None:
        # updating the position        
        self.position   = msg.pose.pose.position.z      
        ...
        self.publish_position_msg(position=self.position, timestamp=timestamp)  
    ...
    def publish_thrust_msg(self, thrust: float, timestamp: rclpy.time.Time) -> None:
        ...
        # we want to set the vertical thrust exlusively. In the bluerovs defined coordination-system thats z-axis facing up, same as the pool coord.
        # mask out xy-components
        msg.ignore_x = True
        msg.ignore_y = True
        msg.ignore_z = False
    ...
    # publishing the current postion
    def publish_position_msg(self, position: float, timestamp: rclpy.time.Time)-> None:
        msg = Float64Stamped()
        msg.data = position
        msg.header.stamp = timestamp.to_msg()
    ...
    def compute_control_output(self) -> float:     # Code of the PID controller
        ...
        if(     (self.position < -0.8 and thrust < 0.0) 
           or   (self.position > -0.1 and thrust > 0.0)):
            return 0.0
